...
def _extract_comm_outputs(
    ...
    Optional[List[np.ndarray]],  # adj_per_round: list of (N,N) per round
    Optional[np.ndarray],  # hard_adj: (N,N) binary
...
class WarehouseCommStepRecord:
    ...
    adj_per_round: Optional[List[np.ndarray]] = None  # list of (N,N) soft adj
    hard_adj: Optional[np.ndarray] = None  # (N,N) binary
    messages: Optional[np.ndarray] = None  # (N, msg_dim) pre-GAT
    agg_messages: Optional[np.ndarray] = None  # (N, msg_dim) post-GAT
    logits: Optional[np.ndarray] = None  # (N, num_actions)
