...
from .continuous_coord_eval_data import (
    AgentState,
    CaptureEvent,
    CollisionEvent,
    EpisodeData,
...
def _decode_agent_state(obs: np.ndarray) -> AgentState:
    ...
    vx = float(obs[2])
    vy = float(obs[3])
    return AgentState(px=px, py=py, vx=vx, vy=vy, speed=float(np.sqrt(vx * vx + vy * vy)))
    ...
    def collect(
        ...
        for ep_idx in range(n_episodes):
            ...
            prev_targets = _decode_targets(
                ref_obs, self.num_agents, self.max_targets,
                self.type_dim, self.deadline_avg,
            )
            ...
            for t in range(max_steps):
                ...
                for a in actual_agents:
                    a_obs = np.asarray(next_obs.get(a, obs.get(a)), dtype=np.float32).ravel()
                ...
                cap_events = _detect_captures(
                    prev_targets, curr_targets, rewards_float,
                    t, self.capture_reward,
                )
