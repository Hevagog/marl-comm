...
from .coingame_data import BLUE_AGENT, RED_AGENT, EvalData
...
def _savefig(fig: plt.Figure, path: Path, dpi: int = 300) -> None:
    """Save figure as both PDF and PNG (dissertation quality)."""
...
def plot_heatmaps(data: EvalData, save_path: Path | None = None) -> plt.Figure:
...
def plot_distance_analysis(data: EvalData, save_path: Path | None = None) -> plt.Figure:
