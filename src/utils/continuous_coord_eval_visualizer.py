...
=====================================
Publication-quality evaluation figures for Continuous Coordination agents.
...
from .continuous_coord_eval_data import ACTION_NAMES, EvalData, EpisodeData

matplotlib.use("Agg")

# ─── Style ────────────────────────────────────────────────────────────────────

_AGENT_COLORS = ["#E63946", "#457B9D", "#2A9D8F", "#E9C46A",
                 "#F4A261", "#6D6875", "#A8DADC", "#264653"]
_BG = "#F8F9FA"
_PANEL_BG = "#FFFFFF"
_GOLD = "#F4A261"
_TEAL = "#2A9D8F"
...
def plot_overview(data: EvalData, save_path: Path | None = None) -> plt.Figure:
    ...
    parts = ax.violinplot([returns], positions=[0], widths=0.6,
                          showmedians=True, showextrema=True)
    for pc in parts["bodies"]:
        pc.set_facecolor(_TEAL)
        pc.set_alpha(0.6)
    parts["cmedians"].set_color("black")
    ax.scatter(np.zeros(n_ep) + np.random.uniform(-0.05, 0.05, n_ep),
               returns, alpha=0.4, s=15, color=_TEAL, zorder=4)
    ax.axhline(np.mean(returns), color=_GOLD, linewidth=1.5, linestyle="--",
    ...
    ax.hist(caps, bins=max(1, max(caps) + 1), color=_TEAL, edgecolor="white",
            alpha=0.8, rwidth=0.85)
    ax.axvline(np.mean(caps), color=_GOLD, linewidth=1.8, linestyle="--",
    ...
    ax.axvline(np.mean(coll_rates), color=_GOLD, linewidth=1.8, linestyle="--",
    ...
    if bonuses:
        ...
        ax.axvline(np.mean(bonuses), color="black", linewidth=1.5, linestyle="--",
                   label=f"Mean={np.mean(bonuses):.2f}")
        ax.legend(fontsize=8)
    else:
        ax.text(0.5, 0.5, "No capture events", ha="center", va="center",
    ...
    ax.fill_between(t, mean_ret - std_ret, mean_ret + std_ret,
                    color=_TEAL, alpha=0.2, label="±1 std")
    ax.set_xlabel("Step")
    ...
    bars = ax.bar(range(9), action_dist * 100, color=colors_bar, edgecolor="white",
                  alpha=0.85)
    ...
    ax.axhline(100 / 9, color="grey", linestyle="--", linewidth=1, label="Uniform (11.1%)")
    ax.legend(fontsize=8)
...
def plot_spatial_heatmaps(
    ...
    for idx, agent in enumerate(agents):
        ...
        im = ax.imshow(hmap, cmap=cmap, vmin=0, vmax=1,
                       origin="upper", extent=[0, 1, 0, 1], aspect="auto")
        ...
        if cap_positions:
            ...
            ax.scatter(cpx, cpy, c="gold", s=30, zorder=5, marker="*",
...
def plot_capture_analysis(
    ...
    if all_cap_steps:
        ...
        ax.text(0.5, 0.5, "No captures recorded", ha="center", va="center",
    ...
    if all_cap_steps and all_sync:
        t_norm = [s / data.max_cycles for s in all_cap_steps]
        sc = ax.scatter(t_norm, all_sync, c=all_k, cmap="plasma",
    ...
    if all_k:
        ...
        ax.bar([str(k) for k in unique_k], counts, color=colors, edgecolor="white",
               alpha=0.85)
    ...
    ax.plot(np.arange(len(smoothed)) + window // 2, smoothed, color=_GOLD,
            linewidth=2, label=f"Smoothed (w={window})")
    ax.axhline(np.mean(caps_per_ep), color="#E63946", linewidth=1.5, linestyle="--",
...
def plot_coordination_dynamics(
    ...
    fig, axes = plt.subplots(2, 2, figsize=(14, 10), constrained_layout=True)
    fig.suptitle("Coordination Dynamics Over Episode Steps",
                 fontsize=14, fontweight="bold")
    ...
    ax.plot(t[valid], mean_d[valid], color=_TEAL, linewidth=2,
            label="Mean inter-agent dist")
    ...
    ax.fill_between(np.arange(data.max_cycles), 0, smoothed, color="#E63946", alpha=0.15)
    ax.set_xlabel("Step")
    ...
    ax.axhline(np.log2(9), color="grey", linestyle="--", linewidth=1,
               label="Max entropy (uniform)")
    ax.set_xlabel("Step")
    ax.set_ylabel("Action entropy (bits)")
    ax.set_title("Joint-Action Coordination Entropy Over Time\n(low = coordinated, high = diverse)")
    ax.legend(fontsize=8)
    ...
    for ep_i, ep in enumerate(data.episodes):
        for step in ep.steps:
            ...
            for a_state in step.agent_states.values():
                min_d = min(
                    math.sqrt((a_state.px - tgt.abs_x) ** 2 + (a_state.py - tgt.abs_y) ** 2)
    ...
    if valid.any():
        ax.plot(t[valid], mean_near[valid], color="#6D6875", linewidth=2,
                label="Mean nearest-target dist")
        ax.fill_between(t[valid], (mean_near - std_near)[valid],
                        (mean_near + std_near)[valid], color="#6D6875", alpha=0.2)
    ax.set_xlabel("Step")
    ax.set_ylabel("Distance to nearest active target")
    ax.set_title("Mean Proximity to Nearest Active Target\n(lower = agents approaching targets)")
    ax.legend(fontsize=8)
...
def plot_trajectories(
    ...
    fig, axes = plt.subplots(rows, cols, figsize=(cols * 5, rows * 5),
                              constrained_layout=True)
    fig.suptitle("Sample Episode Trajectories (continuous [0,1]² space)",
                 fontsize=14, fontweight="bold")
    ...
    for ep_idx, ep in enumerate(episodes):
        ...
        for ev in ep.capture_events:
            if ev.step < len(ep.steps):
                ...
                tgt = ep.steps[ev.step].targets[ev.target_idx] if ev.target_idx < len(ep.steps[ev.step].targets) else None
                ...
                if ev.step > 0:
                    ...
                    if ev.target_idx < len(prev_targets) and prev_targets[ev.target_idx].active:
                        tgt = prev_targets[ev.target_idx]
                        circle = plt.Circle(
                            (tgt.abs_x, tgt.abs_y), 0.04,
                            fill=True, color=_GOLD, alpha=0.3, zorder=2,
                        )
                        ax.add_patch(circle)
                        ax.scatter(tgt.abs_x, tgt.abs_y, c=_GOLD, s=80, zorder=5,
        ...
        for a_idx, agent in enumerate(agents):
            ...
            # Start/end markers
            ax.scatter(*path[0], color=color, s=80, zorder=6, marker="s",
                       edgecolors="white", linewidths=0.5)
            ax.scatter(*path[-1], color=color, s=80, zorder=6, marker="D",
                       edgecolors="white", linewidths=0.5)
    ...
    # Legend
    patches = [mpatches.Patch(color=_agent_color(a, agents),
               label=f"Agent {i}") for i, a in enumerate(agents)]
    patches += [
        plt.Line2D([0], [0], marker="s", color="w", markerfacecolor="grey",
                   markersize=8, label="Start"),
        plt.Line2D([0], [0], marker="D", color="w", markerfacecolor="grey",
                   markersize=8, label="End"),
        plt.Line2D([0], [0], marker="*", color="w", markerfacecolor=_GOLD,
                   markersize=10, label="Capture zone"),
    ]
...
def plot_target_lifecycle(
    ...
    ax.set_title("Mean Active Target Count Over Episode\n(higher pressure = more targets)")
    ...
    stats = {
        ...
        "Mean sync": float(np.mean(data.sync_bonuses())) if data.sync_bonuses() else 0.0,
    }
    keys = list(stats.keys())
    vals = list(stats.values())
    colors_bar = [_TEAL, _TEAL, "#E63946", _GOLD]
    bars = ax.bar(range(len(keys)), vals, color=colors_bar, edgecolor="white", alpha=0.85)
...
def save_all_cc_figures(
    ...
    plot_coordination_dynamics(data, save_path=out / f"{pfx}4_coordination_dynamics.png")
    plot_trajectories(data, n_episodes=min(6, len(data)),
                      save_path=out / f"{pfx}5_trajectories.png")
