    ...
    def delivery_steps(self, agent: str) -> list[int]:
        ...
        for step in self.steps:
            ...
            if state is not None:
                if state.total_deliveries > prev:
                    events.append(step.step)
                    prev = state.total_deliveries
        return events

    def battery_series(self, agent: str) -> np.ndarray:
        return np.array(
            [s.agent_states[agent].battery for s in self.steps if agent in s.agent_states],
            dtype=np.float32,
        )

    def phase_series(self, agent: str) -> np.ndarray:
        return np.array(
            [s.agent_states[agent].phase for s in self.steps if agent in s.agent_states],
            dtype=np.int32,
        )

    def cumulative_deliveries_series(self) -> np.ndarray:
        """Total deliveries summed across all agents, per step."""
        return np.array(
            [sum(s.total_deliveries for s in step.agent_states.values()) for step in self.steps],
            dtype=np.float32,
        )
        ...
        for ep in self.episodes:
            for step in ep.steps:
                ...
                if not active:
                    continue
                for p in range(4):
                    phase_mat[t, p] += sum(1 for s in active if s.phase == p) / len(active)
                count_mat[t] += 1
