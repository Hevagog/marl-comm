...
class EvalData:
    ...
    def mean_cumulative_return_over_time(self) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    def mean_inter_agent_dist_over_time(self) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        mat = np.full((len(self.episodes), self.max_cycles), np.nan)
        for i, ep in enumerate(self.episodes):
