...
def run_commformer_analysis(
    ...
    """
    from utils.commformer.analysis import CommFormerCommCollector
...
def _run_cc_perf(runner, n_episodes, output_dir, exp_name, env_cfg):
    ...
    print(f"\n[CommFormer ContinuousCoord Perf] Collecting {n_episodes} episodes …")
    data = collector.collect(
        env=runner._env, agent=runner._agent, n_episodes=n_episodes
    )
...
def _run_coingame_perf(runner, n_episodes, output_dir, exp_name, env_cfg):
    ...
    print(f"\n[CommFormer CoinGame Perf] Collecting {n_episodes} episodes …")
    data = collector.collect(
        env=runner._env, agent=runner._agent, n_episodes=n_episodes
    )
