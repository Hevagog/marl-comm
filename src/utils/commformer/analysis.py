...
def compute_information_flow(data: CommFormerAnalysisData) -> dict[str, Any]:
    """Random-walk based information flow analysis of the learned graph.

    Models information flow as a random walk on the directed adjacency and
    computes the stationary distribution (PageRank-style).  Agents with high
    stationary probability are central hubs in the communication network.

    Also computes the mixing time (number of steps to approach stationarity)
    as a proxy for how quickly information propagates across the graph.

    Returns
    -------
    dict with keys:
      stationary    : (N,) float — PageRank-style stationary distribution.
      mixing_steps  : int — approximate mixing time.
      hub_agent     : int — agent index with highest stationary probability.
      row_stochastic: (N, N) float — row-normalised transition matrix.
    """
    adj = data.hard_adj
    if adj is None:
        n = data.num_agents
        return {
            ...
            "mixing_steps": 0,
            "hub_agent": 0,
            "row_stochastic": np.eye(n) / n,
        }

    n = adj.shape[0]
    # Row-normalise: P[i,j] = prob of information moving from i to j
    ...
    # Mixing time: power-iterate and measure TV distance from stationarity
    p0 = np.zeros(n)
    p0[0] = 1.0
    mixing = 0
    for step in range(1, 50):
        p0 = p0 @ P
        if np.abs(p0 - pi).max() < 0.05:
            mixing = step
            break
    if mixing == 0:
        mixing = 50  # did not converge in 50 steps

    return {
        "stationary": pi,
        "mixing_steps": mixing,
        "hub_agent": int(np.argmax(pi)),
        "row_stochastic": P,
    }


def compute_laplacian_spectrum(data: CommFormerAnalysisData) -> dict[str, Any]:
    """Eigenvalues of the graph Laplacians (unnormalised and normalised).

    The Fiedler value (λ₁ of normalised Laplacian) is the algebraic
    connectivity of the graph: 0 = disconnected, high = well-connected.

    Returns
    -------
    dict with keys:
      unnorm_eigenvalues  : (N,) eigenvalues of L = D − A.
      norm_eigenvalues    : (N,) eigenvalues of L_norm = I − D^{-½} A D^{-½}.
      fiedler_value       : float — second-smallest normalised Laplacian eigenvalue.
      algebraic_connectivity : same as fiedler_value (alias).
      spectral_gap        : λ₂ − λ₁ of normalised Laplacian.
    """
    adj = data.hard_adj
    if adj is None:
        n = data.num_agents
        return {
            "unnorm_eigenvalues": np.zeros(n),
            "norm_eigenvalues": np.zeros(n),
            "fiedler_value": 0.0,
    ...
    L = D - adj
    L_eigs = np.sort(np.real(np.linalg.eigvalsh(L)))

    deg = adj.sum(axis=1)
    d_inv_sqrt = np.where(deg > 0, 1.0 / np.sqrt(np.maximum(deg, 1e-9)), 0.0)
    D_inv_sqrt = np.diag(d_inv_sqrt)
    L_norm = np.eye(n) - D_inv_sqrt @ adj @ D_inv_sqrt
    L_norm_eigs = np.sort(np.real(np.linalg.eigvalsh(L_norm)))

    fiedler = float(L_norm_eigs[1]) if n > 1 else 0.0
    gap = float(L_norm_eigs[2] - L_norm_eigs[1]) if n > 2 else 0.0

    return {
        "unnorm_eigenvalues": L_eigs,
        "norm_eigenvalues": L_norm_eigs,
...
def compute_representation_alignment(
    data: CommFormerAnalysisData,
) -> dict[str, Any]:
    """Cosine similarity between encoder representations for edge vs non-edge agent pairs.

    Tests whether connected agents develop more aligned representations than
    unconnected agents — the expected signature of effective communication:
    when i receives j's messages, their representations should become more similar.

    Returns
    -------
    dict with keys:
      edge_similarities     : (M_edge,) cosine sims for connected pairs.
      non_edge_similarities : (M_no_edge,) cosine sims for unconnected pairs.
      edge_mean             : float.
      non_edge_mean         : float.
      alignment_advantage   : edge_mean − non_edge_mean.
    """
    if data.hard_adj is None or not data.comm_steps():
        return {
            "edge_similarities": np.empty(0),
            "non_edge_similarities": np.empty(0),
            "edge_mean": 0.0,
            "non_edge_mean": 0.0,
            "alignment_advantage": 0.0,
        }
    ...
    non_edge_sims: list[float] = []

    for s in csteps:
        if s.encoder_out is None:
            continue
        enc = s.encoder_out.astype(float)  # (N, D)
        # Normalise row-wise
        ...
        for i in range(n):
            for j in range(n):
                if i == j:
                    continue
                sim = float(enc_norm[i] @ enc_norm[j])
                if adj[i, j] > 0.5:
                    edge_sims.append(sim)
                else:
                    non_edge_sims.append(sim)

    es = np.asarray(edge_sims, dtype=np.float32) if edge_sims else np.empty(0)
    ns = np.asarray(non_edge_sims, dtype=np.float32) if non_edge_sims else np.empty(0)
    em = float(es.mean()) if len(es) > 0 else 0.0
    nm = float(ns.mean()) if len(ns) > 0 else 0.0

    return {
        "edge_similarities": es,
        "non_edge_similarities": ns,
        "edge_mean": em,
        "non_edge_mean": nm,
        "alignment_advantage": em - nm,
    }


def compute_encoder_variance_dynamics(
    data: CommFormerAnalysisData,
) -> dict[int, np.ndarray]:
    """Per-step representation variance per agent over episode time.

    Increasing variance = agent representations are becoming more diverse
    (possibly developing distinct roles or responding to state changes).
    Decreasing variance = representations are converging (possible collapse).

    Returns dict mapping agent_idx → (T,) array of per-step representation variance
    (computed as mean of squared deviations from the per-step mean).
    """
    csteps = data.comm_steps()
    if not csteps:
        return {}

    # Collect per-step enc vectors for each agent
    per_agent: dict[int, list[np.ndarray]] = {i: [] for i in range(data.num_agents)}
    for s in csteps:
        if s.encoder_out is None:
            continue
        for i in range(min(data.num_agents, s.encoder_out.shape[0])):
            per_agent[i].append(s.encoder_out[i].astype(float))

    result: dict[int, np.ndarray] = {}
    for i, vecs in per_agent.items():
        if len(vecs) < 2:
            continue
...
def compute_alpha_concentration(data: CommFormerAnalysisData) -> dict[str, Any]:
    """How concentrated/sparse is the learned α parameter?

    A sparse α (few large values) indicates the model learned selective
    communication partners.  Entropy of the softmax-normalised α row is a
    natural measure: low entropy = concentrated (selective), high entropy =
    diffuse (nearly uniform communication).

    Returns
    -------
    dict with keys:
      row_entropies  : (N,) — per-agent communication selectivity (higher = less selective).
      col_entropies  : (N,) — per-agent receiver selectivity.
      mean_row_entropy : float.
      gini_row       : (N,) Gini coefficient per row (higher = more concentrated).
    """
    alpha = data.alpha
    if alpha is None:
        n = data.num_agents
    ...
    # Softmax per row
    def _row_entropy(mat: np.ndarray) -> np.ndarray:
        entropies = []
        for i in range(mat.shape[0]):
            row = mat[i]
            row_shifted = row - row.max()
            probs = np.exp(row_shifted)
            probs = probs / probs.sum()
            H = -float((probs * np.log(np.maximum(probs, 1e-12))).sum())
            entropies.append(H)
    ...
    def _gini(x: np.ndarray) -> float:
        x = np.sort(np.abs(x))
        n = len(x)
        if n == 0 or x.sum() < 1e-9:
            return 0.0
        idx = np.arange(1, n + 1)
        return float((2 * (idx * x).sum() / (n * x.sum())) - (n + 1) / n)

    row_ent = _row_entropy(alpha)
    col_ent = _row_entropy(alpha.T)
    gini_row = np.asarray([_gini(alpha[i]) for i in range(n)], dtype=np.float32)

    return {
        "row_entropies": row_ent,
        "col_entropies": col_ent,
        "mean_row_entropy": float(row_ent.mean()),
        "gini_row": gini_row,
    }
...
def print_summary(data: CommFormerAnalysisData) -> None:
    ...
    if data.hard_adj is not None:
        ...
        print(f"  Fiedler value (algebraic connectivity): {spec['fiedler_value']:.4f}")
        print(f"  Spectral gap (λ₂ − λ₁):               {spec['spectral_gap']:.4f}")
        print(
            f"  Info-flow hub agent:  A{flow['hub_agent']}  "
            f"(stationary={flow['stationary'][flow['hub_agent']]:.3f})"
        )
        print(f"  Info-flow mixing time: {flow['mixing_steps']} steps")
        print(
            f"  α row entropy (selectivity): {conc['mean_row_entropy']:.3f}  "
            f"(lower = more selective)"
        )
        if len(align["edge_similarities"]) > 0:
            print(
                f"  Rep. alignment: edge pairs={align['edge_mean']:.3f}  "
                f"non-edge={align['non_edge_mean']:.3f}  "
