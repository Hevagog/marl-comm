...
def _agent_colors(N: int) -> list:
    """Return N distinct agent colours, cycling through tab10 for N > 8."""
...
def _save(fig: plt.Figure, out_dir: Path, name: str, dpi: int = 300) -> None:
    save_figure(fig, out_dir, name, dpi=dpi)
...
def _adj_heatmap(
    ...
    ax.set_xticklabels(
        labels, rotation=45 if N > 6 else 0, ha="right" if N > 6 else "center"
    )
...
def fig_pairwise_barplot(data: MAGICWarehouseData, out_dir: Path, prefix: str) -> None:
    ...
    agent_colors = _agent_colors(N)
    pairs = [
        f"{agent_labels[i]}→{agent_labels[j]}"
        for i in range(N)
        for j in range(N)
        if i != j
    ]
    ...
    for r, ax in enumerate(axes):
        ...
        ax.set_ylim(0, 1.0)
        ax.axhline(
            0.5, color="gray", linestyle="--", lw=0.8, alpha=0.6, label="0.5 threshold"
        )
...
def fig_phase_conditional(data: MAGICWarehouseData, out_dir: Path, prefix: str) -> None:
    ...
    phase_conditions = [
        ...
        (
            "Phase 0 dominant\n(≥3 agents searching)",
            lambda s: s.searching_count >= 3,
            "#4C72B0",
        ),
        (
            "Phase 2: Any treating\n(at treatment station)",
            lambda s: s.treating_count >= 1,
            "#E76F51",
        ),
        (
            "Battery emergency\n(any agent <30)",
            lambda s: s.battery_emergency,
            "#C44E52",
        ),
        ("Rescue active\n(any agent dragging)", lambda s: s.any_rescuing, "#8172B2"),
        (
            "High task pressure\n(pending_tasks > 8)",
            lambda s: s.pending_tasks > 8,
            "#DD8452",
        ),
    ]
...
def fig_battery_conditional(
    ...
    conditions = [
        (
            "All batteries safe\n(all > 50)",
            lambda s: all(v > 50 for v in s.battery.values()),
        ),
        (
            "Low battery present\n(any < 50)",
            lambda s: any(v < 50 for v in s.battery.values()),
        ),
...
def fig_comm_trajectory(
    ...
    fig.suptitle(
        "MAGIC Communication Density — Warehouse Episodes",
        fontsize=13,
        fontweight="bold",
    )
...
def fig_context_correlations(
    ...
    fig, axes = plt.subplots(
        nrows, ncols, figsize=(6 * ncols, 5 * nrows), constrained_layout=True
    )
    fig.suptitle(
    ...
    for ax, (key, title, xlabel) in zip(axes_flat, context_vars):
        ...
        ax.bar(
            centres[valid], means[valid], width=width * 0.8, color="#4C72B0", alpha=0.75
        )
        ax.errorbar(
            centres[valid],
            means[valid],
...
def fig_spatial_comm(data: MAGICWarehouseData, out_dir: Path, prefix: str) -> None:
    ...
    roles = data.agent_roles()
    fig, axes = plt.subplots(
        2, N // 2 + 1, figsize=(6 * (N // 2 + 1), 9), constrained_layout=True
    )
    fig.suptitle(
    ...
    ax = axes_flat[N]
    im = ax.imshow(
        global_mean, origin="upper", cmap=CMAP_HEAT, aspect="auto", vmin=0, vmax=1.0
    )
...
def fig_message_pca(
    ...
    proj3, var3, ctx3 = compute_message_pca_with_context(
        data, "battery", use_agg=use_agg
    )
    ax = axes[2]
    if proj3 is not None:
        scatter = ax.scatter(
            proj3[:, 0], proj3[:, 1], c=ctx3, cmap="RdYlBu", s=4, alpha=0.4
        )
...
def fig_rescue_aligned(data: MAGICWarehouseData, out_dir: Path, prefix: str) -> None:
    R = data.num_comm_rounds
    ...
    for r in range(R):
        ...
        if aligned is None:
            for c in range(3):
                axes[r, c].text(
                    0.5, 0.5, "No rescue events found", ha="center", va="center"
                )
                axes[r, c].axis("off")
            continue
...
def fig_interference_effect(
    ...
    out_dir: Path,
    prefix: str,
    treatment_cols: list[int] | None = None,
) -> None:
    """
    ...
    for s in csteps:
        ...
        if not agents_list:
            continue
        dists = [
            _dist_to_treatment(s.positions[a][0], s.positions[a][1])
    ...
    ax.plot(
        t[valid], np.array(bin_means)[valid], color="#E07B39", linewidth=2, label="Mean"
    )
...
def fig_summary_dashboard(data: MAGICWarehouseData, out_dir: Path, prefix: str) -> None:
    ...
    for col, (key, label, color) in enumerate(ctx_vars):
        ...
        if valid.any():
            width = (centres[1] - centres[0]) if len(centres) > 1 else 0.5
            ax.bar(
                centres[valid], means[valid], width=width * 0.8, color=color, alpha=0.75
            )
            ax.errorbar(
                centres[valid],
                means[valid],
    ...
    if pw:
        ...
        summary_lines += [
            "",
            f"Top comm pair: {top_pair[0]}",
            f"  weight: {top_pair[1]:.3f}",
        ]
    ax.text(
        0.05,
        0.95,
...
def save_all_magic_warehouse_figures(
    ...
    if not data.has_comm_data:
        print(
            "  WARNING: No communication data found. Only basic figures will be generated."
        )
