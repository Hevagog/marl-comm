...
from utils.shared.style import CMAP_COMM, CMAP_HEAT, save_figure
...
def fig_conditional_adj(
    ...
    conditions = [
        ...
        (
            "None reached goal",
            lambda s: (
                not s.reached.get(AGENT_A, False) and not s.reached.get(AGENT_B, False)
            ),
        ),
        (
            "≥1 agent at goal",
...
def fig_topology_stability(data: MAGICAnalysisData, out_dir: Path, prefix: str) -> None:
    """Per-step Jaccard similarity between consecutive adjacency matrices.

    Reveals whether the communication topology is stable (learned fixed
    routing) or reactive (changes state-dependently each step).
    """
    if not data.has_comm_data:
        return
    ...
    fig.suptitle(
        "Communication Topology Stability\n"
        "Jaccard similarity between consecutive adj matrices\n"
        "(1.0 = identical topology, 0.0 = completely different)",
        fontsize=11,
        y=1.04,
    )

    for r in range(min(R, 2)):
        sims = compute_topology_stability(data, r)
        color = PALETTE["a"] if r == 0 else PALETTE["b"]
        lbl = f"Round {r + 1}"

        # Left: timeline
        axes[0].plot(sims, color=color, alpha=0.7, linewidth=1.2, label=lbl)

        # Right: histogram
        lo = float(np.percentile(sims, 1)) if len(sims) > 0 else 0.0
        hi = float(np.percentile(sims, 99)) + 1e-6 if len(sims) > 0 else 1.0
        axes[1].hist(
            sims,
            bins="auto",
            range=(lo, hi),
    ...
    axes[0].set_xlabel("Step pair index (t, t+1)")
    axes[0].set_ylabel("Jaccard similarity")
    axes[0].set_ylim(0, 1.05)
    axes[0].axhline(0.5, color="gray", linewidth=0.8, linestyle="--", alpha=0.6)
    axes[0].legend(fontsize=9)
    axes[0].set_title("Stability over episode time")

    axes[1].set_xlabel("Jaccard similarity")
    axes[1].set_ylabel("Density")
    axes[1].set_title("Distribution of per-step stability")
    axes[1].legend(fontsize=9)

    fig.tight_layout()
    _save(fig, out_dir, f"{prefix}_topology_stability")


def fig_spectral_properties(
    data: MAGICAnalysisData, out_dir: Path, prefix: str
) -> None:
    """Spectral analysis of the mean communication graph.

    Shows:
    - Eigenvalues of the mean adjacency (spectral radius governs message propagation).
    - Normalised Laplacian eigenvalues (Fiedler value = algebraic connectivity).
    """
    if not data.has_comm_data:
        return
    ...
    fig, axes = plt.subplots(1, 2, figsize=(11, 4.5))
    fig.suptitle(
        "Spectral Properties of the Mean Communication Graph",
        fontsize=11,
    )

    colors = [PALETTE["a"], PALETTE["b"], PALETTE["neutral"]]
    for r in range(R):
        sp = compute_spectral_properties(data, r)
        c = colors[r % len(colors)]
        eigs = sp["eigenvalues"]
        N = len(eigs)
        # Adjacency eigenvalues
        axes[0].scatter(range(N), eigs, color=c, s=60, label=f"Round {r + 1}", zorder=4)
        axes[0].plot(range(N), eigs, color=c, alpha=0.4, linewidth=1)

        # Laplacian eigenvalues
        L_eigs = sp["laplacian_eigenvalues"]
        axes[1].scatter(
            range(N),
            L_eigs,
            color=c,
        ...
        axes[1].plot(range(N), L_eigs, color=c, alpha=0.4, linewidth=1)

        print(
            f"  [Spectral R{r + 1}] radius={sp['spectral_radius']:.3f}  "
            f"Fiedler={sp['fiedler_value']:.3f}"
        )

    axes[0].axhline(0, color="gray", linewidth=0.6, linestyle="--")
    axes[0].set_xlabel("Eigenvalue index")
    axes[0].set_ylabel("Eigenvalue (real part)")
    axes[0].set_title("Adjacency eigenvalues\n(spectral radius = max |λ|)")
    axes[0].legend(fontsize=9)

    axes[1].axhline(0, color="gray", linewidth=0.6, linestyle="--")
    axes[1].set_xlabel("Eigenvalue index")
    axes[1].set_ylabel("Laplacian eigenvalue")
    axes[1].set_title(
        "Normalised Laplacian eigenvalues\n(λ₁ = Fiedler; 0 = disconnected)"
    )
    axes[1].legend(fontsize=9)

    fig.tight_layout()
    _save(fig, out_dir, f"{prefix}_spectral_properties")


def fig_role_distribution(data: MAGICAnalysisData, out_dir: Path, prefix: str) -> None:
    """Stacked bar chart of agent communication roles per comm round.

    Roles: broadcaster, receiver, relay, isolated.
    Shows which agents tend to occupy which structural roles.
    """
    if not data.has_comm_data:
        return

    N = 2  # default; overridden from actual adj
    csteps = data.comm_steps()
    if csteps and csteps[0].adj_per_round:
        N = csteps[0].adj_per_round[0].shape[0]

    labels = _TICK[:N] if N <= 2 else [f"A{i}" for i in range(N)]
    R = data.num_comm_rounds
    roles = ("broadcaster", "receiver", "relay", "isolated")
    role_colors = ["#4C72B0", "#DD8452", "#55A868", "#AAAAAA"]

    fig, axes = plt.subplots(1, R, figsize=(max(5, 3 * R), 4.5), squeeze=False)
    fig.suptitle(
        "Agent Communication Role Distribution\n"
        "(fraction of steps in each structural role)",
        fontsize=11,
        y=1.04,
    )
    ...
    for r in range(R):
        ax = axes[0, r]
        role_fracs = compute_role_distribution(data, r)
        if not role_fracs:
        ...
        x = np.arange(N)
        bottom = np.zeros(N)
        for role, rc in zip(roles, role_colors):
            fracs = role_fracs.get(role, np.zeros(N))
            ax.bar(
                x,
                fracs,
                bottom=bottom,
    ...
    fig.tight_layout()
    _save(fig, out_dir, f"{prefix}_role_distribution")


def fig_edge_bimodality(data: MAGICAnalysisData, out_dir: Path, prefix: str) -> None:
    """Edge weight distribution and bimodality analysis.

    A well-trained MAGIC Scheduler should produce bimodal edge weight
    distributions (near 0 = don't communicate; near 1 = communicate).
    The bimodality coefficient BC > 0.555 indicates bimodal distribution.
    """
    if not data.has_comm_data:
        return

    R = data.num_comm_rounds
    fig, axes = plt.subplots(1, R, figsize=(5 * R, 4.5), squeeze=False)
    fig.suptitle(
        "Edge Weight Distribution — Is Communication Hard or Soft?\n"
        "(bimodal ≈ learned hard routing; unimodal ≈ soft/uniform routing)",
        fontsize=11,
        y=1.04,
    )
    ...
    for r in range(R):
        ...
        bm = compute_edge_bimodality(data, r)
        bc = bm["bimodality_coeff"]

        lo = float(np.percentile(weights, 0.5))
        hi = float(np.percentile(weights, 99.5)) + 1e-6
        ax.hist(
            weights,
            bins=40,
            range=(lo, hi),
        ...
        bi_label = "bimodal (BC > 0.555)" if bc > 0.555 else "not clearly bimodal"
        ax.set_title(
            f"Round {r + 1}  —  BC = {bc:.3f} ({bi_label})\n"
            f"mean={bm['mean_weight']:.2f}  "
            f"frac>0.5={bm['frac_above_half']:.2f}  "
            f"frac<0.1={bm['frac_below_tenth']:.2f}"
        )
        ax.set_xlabel("Off-diagonal edge weight")
        ax.set_ylabel("Density")
        ax.legend(fontsize=9)

    fig.tight_layout()
    _save(fig, out_dir, f"{prefix}_edge_bimodality")


def fig_round_agreement(data: MAGICAnalysisData, out_dir: Path, prefix: str) -> None:
    """Jaccard agreement between communication rounds R1 and R2.

    Shows how much the topology decided in round 1 is preserved or refined
    in round 2.  High agreement → rounds are redundant.  Low agreement →
    rounds refine the routing strategy sequentially.
    """
    if not data.has_comm_data or data.num_comm_rounds < 2:
        return

    agreements = compute_round_agreement(data, r1=0, r2=data.num_comm_rounds - 1)
    if len(agreements) == 0:
        return

    fig, axes = plt.subplots(1, 2, figsize=(11, 4.2))
    fig.suptitle(
        f"Round 1 vs Round {data.num_comm_rounds} Topology Agreement\n"
        "(Jaccard similarity of hard-thresholded adjacencies)",
        fontsize=11,
    )

    axes[0].plot(agreements, color=PALETTE["neutral"], linewidth=1.2)
    axes[0].axhline(
        agreements.mean(),
        color="black",
        linewidth=1.5,
        linestyle="--",
        label=f"Mean {agreements.mean():.2f}",
    )
    axes[0].set_xlabel("Step index")
    axes[0].set_ylabel("Jaccard agreement")
    axes[0].set_ylim(0, 1.05)
    axes[0].set_title("Agreement over episode steps")
    axes[0].legend(fontsize=9)

    lo = float(np.percentile(agreements, 1))
    hi = float(np.percentile(agreements, 99)) + 1e-6
    axes[1].hist(
        agreements,
        bins="auto",
        range=(lo, hi),
    ...
    axes[1].axvline(agreements.mean(), color="black", linewidth=1.5, linestyle="--")
    axes[1].set_xlabel("Jaccard agreement")
    axes[1].set_ylabel("Count")
    axes[1].set_title("Distribution of round agreement")

    fig.tight_layout()
    _save(fig, out_dir, f"{prefix}_round_agreement")


def fig_message_information_gain(
    data: MAGICAnalysisData, out_dir: Path, prefix: str
) -> None:
    """How much do messages change after GAT aggregation (per agent)?

    Information gain = ||agg_msg - raw_msg|| / ||raw_msg||.
    High gain → GAT transforms messages significantly (rich neighbour info).
    Low gain  → aggregation barely changes the message (sparse communication).
    """
    if not data.has_comm_data:
        return

    gains = compute_message_information_gain(data)
    if not gains:
        return
    ...
    fig, axes = plt.subplots(1, 2, figsize=(12, 4.5))
    fig.suptitle(
        "Message Information Gain from GAT Aggregation\n"
        "‖agg_msg − raw_msg‖ / ‖raw_msg‖  "
        "(higher = GAT changed the message more)",
        fontsize=11,
        y=1.04,
    )

    # Left: per-agent gain over episode steps
    for i, (agent_idx, gain_arr) in enumerate(gains.items()):
        ...
        axes[0].plot(
            ...
            alpha=0.7,
            linewidth=1.2,
            label=labels[i] if i < len(labels) else f"A{agent_idx}",
        )
    axes[0].set_xlabel("Step index (across all episodes)")
    axes[0].set_ylabel("Normalised L2 change")
    axes[0].set_title("Gain over episode time")
    axes[0].legend(fontsize=9)

    # Right: distribution per agent (violin or box)
    all_gains = [gains[i] for i in sorted(gains.keys())]
    vlns = axes[1].violinplot(all_gains, positions=range(N), showmedians=True)
    for j, body in enumerate(vlns["bodies"]):
        body.set_facecolor(list(PALETTE.values())[j % len(PALETTE)])
        body.set_alpha(0.65)
    axes[1].set_xticks(range(N))
    axes[1].set_xticklabels(labels)
    axes[1].set_ylabel("Normalised information gain")
    axes[1].set_title("Distribution per agent")

    fig.tight_layout()
    _save(fig, out_dir, f"{prefix}_message_information_gain")


def fig_graph_density_profile(
    data: MAGICAnalysisData, out_dir: Path, prefix: str
) -> None:
    """Smoothed communication density profile over episode time (per round).

    Shows when during an episode agents communicate most intensely.
    Peaks may correspond to key coordination events.
    """
    if not data.has_comm_data:
        return
    ...
    fig, ax = plt.subplots(figsize=(11, 4.2))
    ax.set_title(
        "Communication Density Profile Over Episode Time\n"
        "(rolling-mean smoothed; peaks = high-communication moments)",
        fontsize=11,
    )

    colors = [PALETTE["a"], PALETTE["b"], PALETTE["neutral"]]
    for r in range(R):
        steps, smoothed, raw = compute_graph_density_over_time(data, r)
        if len(steps) == 0:
            continue
        c = colors[r % len(colors)]
        ax.plot(steps, raw, color=c, alpha=0.2, linewidth=0.7)
        ax.plot(
            steps, smoothed, color=c, linewidth=2.0, label=f"Round {r + 1} (smoothed)"
        )

    ax.set_xlabel("Episode step")
    ax.set_ylabel("Mean off-diagonal edge weight")
    ax.set_ylim(0, 1.05)
    ax.axhline(0.5, color="gray", linewidth=0.8, linestyle="--", alpha=0.5)
    ax.legend(fontsize=9)
    fig.tight_layout()
    _save(fig, out_dir, f"{prefix}_density_profile")


def save_all_magic_figures_extended(
    data: MAGICAnalysisData,
    output_dir: str = "eval_plots/magic",
    prefix: str = "magic",
) -> None:
    """Generate and save all MAGIC figures including extended graph analysis.

    Calls the original ``save_all_magic_figures`` plus the new extended figures.
    """
    out_dir = Path(output_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    save_all_magic_figures(data, output_dir=output_dir, prefix=prefix)

    extended_fns = [
        ("Topology stability", fig_topology_stability),
        ("Spectral properties", fig_spectral_properties),
        ("Role distribution", fig_role_distribution),
        ("Edge bimodality", fig_edge_bimodality),
        ("Round agreement", fig_round_agreement),
        ("Message information gain", fig_message_information_gain),
        ("Graph density profile", fig_graph_density_profile),
    ]

    print("\n  Extended MAGIC graph analysis figures:")
    for label, fn in extended_fns:
        print(f"  Generating: {label} …", end=" ", flush=True)
        try:
            fn(data, out_dir, prefix)
            print("done")
    ...
    print(f"\n  Extended figures saved to: {out_dir.resolve()}")
