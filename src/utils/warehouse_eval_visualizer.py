...
def fig_throughput(data: EvalData, out_dir: Path, prefix: str) -> None:
    ...
    fig.suptitle("Warehouse Throughput & Delivery Efficiency", fontsize=13, fontweight="bold")
    ...
    ax.plot(t_vals[valid], mean_rate[valid], color="#2A9D8F", linewidth=2, label="Mean rate")
    ax.fill_between(t_vals[valid],
    ...
    ax.scatter(lengths, deliveries, c=efficiency, cmap="viridis", s=40,
...
def fig_action_congestion(data: EvalData, out_dir: Path, prefix: str) -> None:
    ...
    fig.suptitle("Action Distribution & Spatial Congestion", fontsize=13, fontweight="bold")
    gs = fig.add_gridspec(2, 3)
    ...
    for a_idx, agent in enumerate(data.agents):
        ...
        ax.bar(x + offset, freq * 100, bar_w * 0.9,
               color=_AGENT_COLORS[a_idx % len(_AGENT_COLORS)], alpha=0.82,
               label=agent)
    ax.set_xticks(x)
    ...
    ax.axhline(100 / len(ACTION_NAMES), color="grey", linestyle="--", linewidth=1, alpha=0.5)
    ...
    bars = ax.bar([f"A{i}" for i in range(n_agents)], rescue_fracs,
                  color=_AGENT_COLORS[:n_agents], alpha=0.85)
    ...
    if len(data.agents) >= 2:
        ...
        colors = [_AGENT_COLORS[i % len(_AGENT_COLORS)] for i in range(len(pair_labels))]
        bars = ax.bar(pair_labels, congestion_counts, color=colors, alpha=0.85, edgecolor="white")
        for bar, v in zip(bars, congestion_counts):
            ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.1,
                    f"{v:.1f}%", ha="center", fontsize=8)
        ax.set_ylabel("% of steps within 1-cell")
        ax.set_title("Pairwise Agent Co-location Rate\n(% steps where agents within 1 cell = congestion metric)")
        ax.axhline(float(np.mean(congestion_counts)), color="#C44E52", linestyle="--",
                   linewidth=1.5, label=f"Mean={np.mean(congestion_counts):.1f}%")
...
def fig_scenario_metrics(data: EvalData, out_dir: Path, prefix: str) -> None:
    ...
    ax.bar(xpos, means, yerr=stds, capsize=4, color=[
        _AGENT_COLORS[i % len(_AGENT_COLORS)] for i in range(len(data.agents))
    ], alpha=0.85)
    ax.set_xticks(xpos)
    ax.set_xticklabels([f"A{i}" for i in range(len(data.agents))])
    ax.set_ylabel("Deliveries / episode")
    ax.set_title(
        "Per-Agent Deliveries (true count)\n"
        f"Total mean/ep: {sum(means):.1f}"
    )
    ...
    if any(total_deliveries):
        ax.hist(sync_frac, bins=10, range=(0.0, 1.0), color="#9467BD",
                alpha=0.85, edgecolor="white")
        ax.axvline(float(np.mean(sync_frac)), color="#E07B39", linestyle="--",
                   linewidth=2,
    ...
    if mean_occ.sum() > 0:
        ...
        ax.text(0.5, 0.5, "rendezvous disabled or never visited",
