"""MAMHM-enhanced MAPPO agent with autoregressive action selection.

Extends CategoricalMAPPO to stack all agents' observations and call
the shared MAMHMPolicyNet, which uses Mamba-based encoder-decoder plus
a Hopfield Memory Bank for associative memory augmentation.

The agent class is nearly identical to MAMMAPPO since the memory bank
is integrated inside the policy network. The only difference is using
MAMHMPolicyNet instead of MAMPolicyNet.
...
class MAMHMMAPPO(CategoricalMAPPO):
    """MAPPO agent with Multi-Agent Mamba + Hopfield Memory policy.
    ...
    actions autoregressively via Mamba + CrossMamba + HopfieldMemoryBank.
    """
    ...
    def act(
        ...
        ``batch_size = num_agents * num_envs``, allowing the MAMHM
        encoder-decoder to run over the full agent group.

        An ``ar_key`` is injected into the inputs so MAMHMPolicyNet
    ...
    def _shuffle_buffer_indices(self, buffer_size: int) -> np.ndarray:
        ...
        MAMHMPolicyNet's forward pass requires consecutive rows in the
