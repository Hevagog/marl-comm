...
@dataclass
class MambaArgs:
    """Hyperparameters for single Mamba block."""
...
class BiMambaBlock(nn.Module):
    """Bidirectional Mamba: forward + reverse with SEPARATE parameters.

    This version uses two independent ``MambaBlock`` instances so the
    network can learn distinct forward-context and backward-context
    representations, as described in bidirectional SSM literature
    (Vision Mamba, MambaND, etc.) and implicitly required by the MAM
    encoder design (Daniel et al. 2024, §3.1).
    """

    args: MambaArgs

    def setup(self) -> None:
        self.fwd_block = MambaBlock(self.args)
        self.bwd_block = MambaBlock(self.args)

    def __call__(self, x: jax.Array) -> jax.Array:
        x_fwd = self._one_dir(self.fwd_block, x)
        x_rev = self._one_dir(self.bwd_block, jnp.flip(x, axis=1))
