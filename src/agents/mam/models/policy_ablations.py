"""MAM ablation policy networks.

Ablation 1 / Ablation 2: Encoder-only policy.
- BiMamba encoder (same as MAMPolicyNet.Encoder)
- Per-agent MLP head producing action logits
- No decoder, no autoregressive mode, no teacher forcing

This is also the "MAPPO + BiMamba" framing (Ablation 2): the same
architecture but used as a drop-in MAPPO policy — the agent processes
all-agent observations jointly via BiMamba, then acts independently.
"""
...
class MAMEncoderOnlyPolicyNet(CategoricalMixin, Model):
    """BiMamba encoder + per-agent MLP head, no autoregressive decoder.

    Architecture:
      obs (B, obs_dim)
        → reshape (groups, n, obs_dim)
        → BiMamba encoder → obs_rep (groups, n, n_embd)
        → MLP head per agent → logits (groups, n, act_dim)
        → reshape (B, act_dim)

    Training and rollout both use the same parallel forward pass.
    ``taken_actions`` and ``ar_key`` are ignored.
    """
    ...
    def __call__(self, inputs: Mapping[str, Any], role: str = ""):
        ...
        assert b >= n and b % n == 0, (
            f"MAMEncoderOnlyPolicyNet requires batch_size ({b}) divisible by "
            f"num_agents ({n})."
        )

        groups = b // n
        obs_grouped = x.reshape(groups, n, -1)
        obs_rep = self._encoder(obs_grouped)  # (groups, n, n_embd)
    ...
    def init_state_dict(self, role: str, inputs=None, key=None) -> None:
        """Init params with a batch of num_agents observations."""
        if inputs is None:
            obs_dim = self.observation_space.shape[0]
