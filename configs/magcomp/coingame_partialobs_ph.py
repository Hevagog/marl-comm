# fmt: off
"""Coin Game Partial-Obs — MAGIC Persistent Hopfield (PH).

HopfieldStateCell with K=16 prototypes, H=32. Under partial observability
the K prototypes capture partner-visibility states: "partner visible at
range r, coin contested" vs "partner invisible, hold territory". The update
gate tracks mode transitions across the 2-step vision dropout periods.

Carry = (B, 32). EH used T=12 giving (B, 384) — 12× smaller footprint.
"""
...
CONFIG = {
    "experiment": {
        "name": "magcomp_coingame_partialobs_ph", "agent_type": "magic",
        "directory": "runs", "wandb": True,
        "wandb_kwargs": {"project": "marl-comm", "tags": ["magcomp", "coingame_partialobs", "magic_ph"]},
