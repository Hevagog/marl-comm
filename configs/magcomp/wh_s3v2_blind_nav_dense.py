# fmt: off
"""S3-v2 Blind Navigation — MAGIC Dense (FF encoder).

No temporal integration — sees only the current noisy frame and whatever peer
position the attention graph can route.  Without the GPS (hide_infra_obs=True)
Dense cannot persist discovered infrastructure locations across steps; it must
re-sight infrastructure on every forward pass.  Strongest contrast case for
recurrent/Hopfield variants that integrate over time.
"""
...
CONFIG = {
    "experiment": {
        "name": "magcomp_wh_s3v2_dense", "agent_type": "magic",
        "directory": "runs", "wandb": True,
        "wandb_kwargs": {"project": "marl-comm", "tags": ["magcomp", "wh_s3v2", "magic_dense", "blind_nav"]},
