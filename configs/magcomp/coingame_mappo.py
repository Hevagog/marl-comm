# fmt: off
"""Coin Game — MAPPO baseline.

Mixed-motive 2-agent: pick own coin = +1, steal opponent's = +1 self / -2 to
victim. MAPPO (no comm) cannot signal "I'm going for X" so equilibrium drifts
toward steal-heavy policies. obs=12, state=24, max_cycles=50.
"""
...
CONFIG = {
    "experiment": {
        "name": "magcomp_coingame_mappo", "agent_type": "mappo",
        "directory": "runs", "wandb": True,
        "wandb_kwargs": {"project": "marl-comm", "tags": ["magcomp", "coingame", "mappo"]},
