# fmt: off
"""Phase 2 — GRU full fine-tune transfer to standard (selfish) reward.

USAGE:
    python -m cli --config configs/magcomp/coingame_partialobs_gru_transfer_finetune.py \
        --task train \
        --resume runs/magcomp_coingame_partialobs_gru_altruistic/checkpoints/agent_5000000.pt

Transfer condition: full_finetune (symmetric pairing), GRU control arm.

Purpose: tests whether cooperation persistence under reward-shift is specific
to the Hopfield discrete-attractor geometry, or is a general property of
recurrent memory (i.e., any RNN would show similar behavioral inertia).

Expected result: GRU shows intermediate persistence — more than MAPPO (has
memory), less than HSC frozen_proto (no discrete attractor locking).
"""
...
CONFIG = {
    "experiment": {
        "name": "magcomp_coingame_partialobs_gru_transfer_finetune", "agent_type": "magic",
        "directory": "runs", "wandb": True,
        "wandb_kwargs": {"project": "marl-comm", "tags": ["magcomp", "coingame_partialobs", "magic_gru", "transfer", "full_finetune", "phase2", "symmetric"]},
