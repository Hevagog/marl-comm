# fmt: off
"""S1 Team-sync — MAGIC Persistent Hopfield (PH).

HopfieldStateCell with K=32 prototypes, H=64. The 4-agent warehouse has
~12 task-role combinations per agent (pick/treat/deliver × fast/slow/
high-cap) — K=32 covers the full heterogeneous role space. The GRU gate
decides when to transition modes (e.g., "delivering → idle").

Carry = (B, 64). EH used T=16 giving (B, 1024) — 16× smaller.
"""
...
CONFIG = {
    "experiment": {
        "name": "magcomp_wh_s1_ph", "agent_type": "magic",
        "directory": "runs", "wandb": True,
        "wandb_kwargs": {"project": "marl-comm", "tags": ["magcomp", "wh_s1", "magic_ph"]},
