# fmt: off
"""S2 Rendezvous — MAGIC Episodic Hopfield (EH).

vision_range=1 produces sparse, intermittent peer sightings. EH stores the
last T=16 obs_enc vectors and retrieves by content similarity: when a peer
re-enters the patch, the buffer recovers the prior "saw peer at +Δ" embedding,
giving the GAT scheduler a stable peer-position signal across vision dropouts.
T×H = 16×64 = 1024 carry width.
"""
...
CONFIG = {
    "experiment": {
        "name": "magcomp_wh_s2_eh", "agent_type": "magic",
        "directory": "runs", "wandb": True,
        "wandb_kwargs": {"project": "marl-comm", "tags": ["magcomp", "wh_s2", "magic_eh"]},
