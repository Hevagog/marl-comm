# fmt: off
"""S2 Rendezvous — MAGIC Persistent Hopfield (PH).

vision_range=1 produces sparse peer sightings. HSC K=32 prototypes encode
"peer last seen at region R, rendezvous open/closed, my role". The GRU
gate transitions to the "seek rendezvous" mode when the obs_enc indicates
a peer recently entered the rendezvous patch. Carry = (B, 64).
"""
...
CONFIG = {
    "experiment": {
        "name": "magcomp_wh_s2_ph", "agent_type": "magic",
        "directory": "runs", "wandb": True,
        "wandb_kwargs": {"project": "marl-comm", "tags": ["magcomp", "wh_s2", "magic_ph"]},
