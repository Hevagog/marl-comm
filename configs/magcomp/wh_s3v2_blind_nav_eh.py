# fmt: off
"""S3-v2 Blind Navigation — MAGIC Episodic Hopfield (EH).

Buffer of last T=16 obs_enc vectors.  Without the GPS (hide_infra_obs=True),
content-addressable retrieval can recover the most-similar prior frame where
the agent last had a treatment/goal cell in its 5×5 vision patch — pattern
completion from memory rather than direct sensing.  Different inductive bias
from LSTM/GRU's exponential smoothing.  Degrades if most buffered frames were
collected while still searching (no infrastructure in view), since they will
dominate retrieval.
"""
...
CONFIG = {
    "experiment": {
        "name": "magcomp_wh_s3v2_eh", "agent_type": "magic",
        "directory": "runs", "wandb": True,
        "wandb_kwargs": {"project": "marl-comm", "tags": ["magcomp", "wh_s3v2", "magic_eh", "blind_nav"]},
