# fmt: off
"""S3-v2 Blind Navigation — MAGIC GRU.

Same temporal-integration role as LSTM, lighter cell.  With hide_infra_obs=True
the recurrent state must encode "last known direction to treatment/goal" derived
from past vision patches, rather than receiving it from the GPS block.  GRU
often matches LSTM at RL horizons under TBPTT(1); preferred when capacity is
the bottleneck rather than gradient depth.
"""
...
CONFIG = {
    "experiment": {
        "name": "magcomp_wh_s3v2_gru", "agent_type": "magic",
        "directory": "runs", "wandb": True,
        "wandb_kwargs": {"project": "marl-comm", "tags": ["magcomp", "wh_s3v2", "magic_gru", "blind_nav"]},
