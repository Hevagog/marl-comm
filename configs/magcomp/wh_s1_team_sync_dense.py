# fmt: off
"""S1 Team-sync — MAGIC Dense (feed-forward, no recurrent encoder).

FC obs encoder → msg encoder → GAT scheduler + message processor → action head.
The attention graph can route "I just delivered" to the partner agent in O(1)
hops, enabling it to time its next delivery within the 20-step window.
Baseline for isolating the marginal value of the recurrent encoder variants.
"""
...
CONFIG = {
    "experiment": {
        "name": "magcomp_wh_s1_dense", "agent_type": "magic",
