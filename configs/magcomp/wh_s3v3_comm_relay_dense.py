# fmt: off
"""S3-v3 Comm Relay — MAGIC Dense (FF encoder).

no_comm=False: peer position/battery/carrying/active features populated at
comm_range=5. MAGIC messages route sender's FULL compressed obs (including
5×5 vision patch) to all other agents. Receiver can infer peer infrastructure
sightings — not possible in S3v2 where no_comm=True zeroed peer slots.

MAPPO sees the same scalar peer features but has no cross-agent message
routing; its MLP treats each peer slot as an independent input. The
differentiated information is peer vision content, only accessible through
the MAGIC attention graph.

Dense has no temporal carry — it cannot persist a sighted location after
the infrastructure leaves its vision. Isolates the communication signal
without temporal memory confounds.
"""
...
CONFIG = {
    "experiment": {
        "name": "magcomp_wh_s3v3_dense", "agent_type": "magic",
        "directory": "runs", "wandb": True,
        "wandb_kwargs": {"project": "marl-comm", "tags": ["magcomp", "wh_s3v3", "magic_dense", "comm_relay"]},
