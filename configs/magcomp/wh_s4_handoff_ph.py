# fmt: off
"""S4 Handoff — MAGIC Persistent Hopfield (PH).

Asymmetric agents: speed=(1,3,1,3), capacity=(3,1,3,1). Handoff requires
alternating pick↔drop cycles between fast-light and slow-heavy agents.
K=32 prototypes cover handoff phase combinations. The GRU gate commits
to "I am the carrier" or "I am the depositor" mode until explicitly
contradicted by obs_enc.
"""
...
_ENV = {
    ...
    "num_shelves": 8, "resources_per_shelf": 4,
    ...
    "max_pending_tasks": 10, "penalty_task_expired": -1.0,
...
CONFIG = {
    "experiment": {
        "name": "magcomp_wh_s4_ph", "agent_type": "magic",
        "directory": "runs", "wandb": True,
        "wandb_kwargs": {"project": "marl-comm", "tags": ["magcomp", "wh_s4", "magic_ph"]},
