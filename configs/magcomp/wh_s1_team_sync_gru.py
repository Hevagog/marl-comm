# fmt: off
"""S1 Team-sync — MAGIC GRU (TBPTT(1)).

GRU replaces LSTM. Preferred over LSTM for this setting: no separate cell state
so TBPTT(1) is less penalised by the single-step gradient depth; 2/3 the
parameters for equivalent single-step expressivity. Gate dynamics can still
track recent delivery events in h_t. TBPTT(1) side-buffer is wired; IS ratio
≈ 1.0 at update epoch 0.
"""
...
CONFIG = {
    "experiment": {
        "name": "magcomp_wh_s1_gru", "agent_type": "magic",
        "directory": "runs", "wandb": True,
        "wandb_kwargs": {"project": "marl-comm", "tags": ["magcomp", "wh_s1", "magic_gru"]},
