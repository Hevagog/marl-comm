# fmt: off
"""S3 Packet Loss — MAGIC Persistent Hopfield (PH).

Under load-dependent comm loss, the persistent carry holds the agent's last
confirmed coordination mode rather than trying to reconstruct it from a
dropped-frame buffer (EH degrades when frames in the buffer are corrupted).
K=32 prototypes cover interference-zone behaviors (avoid-zone, wait, reroute).
"""
...
CONFIG = {
    "experiment": {
        "name": "magcomp_wh_s3_ph", "agent_type": "magic",
        "directory": "runs", "wandb": True,
        "wandb_kwargs": {"project": "marl-comm", "tags": ["magcomp", "wh_s3", "magic_ph"]},
