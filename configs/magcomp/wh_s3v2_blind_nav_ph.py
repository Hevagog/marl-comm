# fmt: off
"""S3-v2 Blind Navigation — MAGIC Persistent Hopfield (PH).

hide_infra_obs=True removes the 8-feature GPS (obs[7:15]).
MAGIC PH should outperform MAPPO here:
  - Attention graph routes "I see treatment cell at (+dr, +dc)" to teammates.
  - PH carry persists explored-infrastructure modes across steps.
  - With noisy comms (interference + packet loss), PH is more robust than EH.
"""
...
CONFIG = {
    "experiment": {
        "name": "magcomp_wh_s3v2_ph", "agent_type": "magic",
        "directory": "runs", "wandb": True,
        "wandb_kwargs": {"project": "marl-comm", "tags": ["magcomp", "wh_s3v2", "magic_ph", "blind_nav"]},
