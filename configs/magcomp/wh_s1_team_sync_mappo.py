# fmt: off
"""S1 Team-sync — MAPPO (dense baseline).

Hypothesis: MAPPO's per-agent advantage estimator cannot time deliveries to
fall inside a 20-step window because it has no peer-phase signal. Agents learn
to maximise individual deliveries independently, leaving the team-sync bonus
(+5.0 per coordinated pair) mostly uncaptured. MAGIC variants should open a
gap by routing "I just delivered" through the attention graph.
"""
...
CONFIG = {
    "experiment": {
        "name": "magcomp_wh_s1_mappo", "agent_type": "mappo",
        "directory": "runs", "wandb": True,
        "wandb_kwargs": {"project": "marl-comm", "tags": ["magcomp", "wh_s1", "mappo"]},
