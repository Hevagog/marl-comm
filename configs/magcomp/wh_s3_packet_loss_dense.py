# fmt: off
"""S3 Packet Loss — MAGIC Dense (FF encoder).

GAT scheduler still routes messages but each frame is a corrupted snapshot.
No temporal integration — sees only current noisy frame. Strongest contrast
case for recurrent variants which can filter in the time dimension.
"""
...
CONFIG = {
    "experiment": {
        "name": "magcomp_wh_s3_dense", "agent_type": "magic",
        "directory": "runs", "wandb": True,
        "wandb_kwargs": {"project": "marl-comm", "tags": ["magcomp", "wh_s3", "magic_dense"]},
