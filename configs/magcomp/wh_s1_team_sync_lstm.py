# fmt: off
"""S1 Team-sync — MAGIC LSTM (TBPTT(1)).

Adds an OptimizedLSTMCell between the obs encoder and msg encoder (MAGIC §4.1
Eq. 3). The cell accumulates per-agent hidden state across timesteps, giving
the policy a temporal window over recent obs encodings. For team-sync the cell
can implicitly track "steps since last delivery" without explicit comm, then
modulate messages accordingly. TBPTT(1) is wired via the side-buffer in
MAGICMAPPO — update IS ratio matches rollout IS ratio from epoch 0.

Expected vs Dense: marginal lift (~5–10) if the LSTM learns to retain delivery
timestamps in its hidden state; no lift if obs is Markovian enough for the
single-step FC encoder to carry sufficient context.
"""
...
CONFIG = {
    "experiment": {
        "name": "magcomp_wh_s1_lstm", "agent_type": "magic",
        "directory": "runs", "wandb": True,
        "wandb_kwargs": {"project": "marl-comm", "tags": ["magcomp", "wh_s1", "magic_lstm"]},
