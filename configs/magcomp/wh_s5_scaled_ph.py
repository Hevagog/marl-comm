# fmt: off
"""S5 Scaled — MAGIC Persistent Hopfield (12 agents).

12 agents × 3 heterogeneous types × task phases → K=64 prototypes to
cover the expanded role space. H=64 carry per agent. The scaled scenario
has long episodes (500 steps) where prototype-based mode persistence
matches the timescale of full pick→treat→deliver chains (≈20 steps each).
Carry = (B, 64) vs EH (B, 1024) — 16× smaller, critical at 12 agents.
"""
...
CONFIG = {
    "experiment": {
        "name": "magcomp_wh_s5_ph_v3", "agent_type": "magic",
        "directory": "runs", "wandb": True,
        "wandb_kwargs": {"project": "marl-comm", "tags": ["magcomp", "wh_s5", "v3", "magic_ph"]},
