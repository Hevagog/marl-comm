# fmt: off
"""Coin Game — MAM diagnostic config.

Coin Game: 2 agents, 4 actions, obs=12, state=24, max_cycles=50. Tiny
mixed-motive env — fast iteration, cheap to overfit, easy to spot
no-op collapse (action 0 = stay; if MAM's ≈315 warehouse fixed point
generalises, here it would manifest as both agents pinning a single
action and never picking up coins → reward ≈0).

Why coingame for MAM:
  - 2 agents → AR chain length 2; minimal cross-mamba complexity.
  - Discrete-4 actions → log(4)=1.386 entropy ceiling; collapse is
    visually obvious in wandb.
  - max_cycles=50 → 256 rollouts × 16 envs ÷ 50 ≈ 80 episodes/update;
    healthy reward signal each update.
  - Mixed-motive coordination is the kind of task MAM's joint encoder
    is supposed to help with vs MAPPO.

Knobs taken from v5b (post-NaN-guard) — same architecture defaults as
warehouse_scaled MAM, scaled down for 2 agents:
  - n_embd 128 → 64; d_state 64 → 32; delta_rank 64 → 32
  - learning_rate 1.5e-4 (conservative, same as v5b)
  - ratio_max_threshold 3.0, kl_threshold 0.03, kl_warmup 0.0
  - isfinite-grad guard now lives in categorical_mappo (post-v5b fix)

Success criteria (after 1M steps):
  - mean reward > MAPPO baseline (configs/magcomp/coingame_mappo.py)
  - entropy NOT saturated at log(4) ≈ 1.386
  - Diagnostics_/_grad_nonfinite_skipped: should be 0.0 throughout
    (any non-zero = the NaN bug still latent — catch it visibly).
"""
