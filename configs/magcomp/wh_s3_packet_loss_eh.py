# fmt: off
"""S3 Packet Loss — MAGIC Episodic Hopfield.

Buffer of last T=16 obs_enc vectors. When a frame's msg payload is dropped,
content-addressable retrieval can recover the most-similar prior frame's
embedding (e.g., last clean snapshot pre-interference). Different bias from
LSTM/GRU's exponential smoothing — pattern completion vs running average.
"""
...
CONFIG = {
    "experiment": {
        "name": "magcomp_wh_s3_eh", "agent_type": "magic",
        "directory": "runs", "wandb": True,
        "wandb_kwargs": {"project": "marl-comm", "tags": ["magcomp", "wh_s3", "magic_eh"]},
