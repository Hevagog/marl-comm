# fmt: off
"""Coin Game Partial-Obs — MAGIC GRU trained with altruistic reward mixing.

Phase 1 control baseline.  Same social_welfare_alpha=0.7 as the HSC run,
same architecture otherwise, but with a standard GRU recurrent cell.

Purpose: isolate whether behavioral persistence in Phase 2 is due to the
Hopfield prototype geometry (HSC-specific) vs. generic policy inertia that
any recurrent architecture would exhibit.

Comparison matrix for Phase 2 transfer:
  HSC frozen_proto  — prototypes locked, adaptation via policy head only
  HSC full_finetune — all weights adaptive
  GRU full_finetune — no structured attractors; inertia from GRU hidden state
  MAPPO             — no memory; pure reward-following baseline
"""
...
CONFIG = {
    "experiment": {
        "name": "magcomp_coingame_partialobs_gru_altruistic", "agent_type": "magic",
        "directory": "runs", "wandb": True,
        "wandb_kwargs": {"project": "marl-comm", "tags": ["magcomp", "coingame_partialobs", "magic_gru", "altruistic", "phase1"]},
