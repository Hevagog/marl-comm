# fmt: off
"""Coin Game Partial-Obs — MAGIC GRU.

GRU variant of the recurrent MAGIC encoder.  Fewer parameters than LSTM
(no separate cell state) while retaining the gating mechanism that lets
the agent decide how much of the previous hidden state to carry forward.

Under partial obs the GRU should learn: when other agent visible → reset
(update hidden to current observation); when invisible → gate-blend old
hidden state with current obs encoder output to maintain an estimate of
where the partner might be.  This is the standard belief-update intuition.
"""
...
CONFIG = {
    "experiment": {
        "name": "magcomp_coingame_partialobs_gru", "agent_type": "magic",
        "directory": "runs", "wandb": True,
        "wandb_kwargs": {"project": "marl-comm", "tags": ["magcomp", "coingame_partialobs", "magic_gru"]},
