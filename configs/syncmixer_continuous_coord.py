# fmt: off
# SyncMixer on continuous_coord.
# Actor: 2-block symmetric-attention token encoder + HopfieldPooling
#        (K=4 queries, β=2.0, pool_dim=32) → broadcast context + per-agent MLP head.
# Critic: shared MLP (MAPPO-style) over global_state + one-hot agent ID.
# PPO knobs match the MAPPO continuous_coord baseline that hits 90 reward.
import jax.numpy as jnp
from skrl.resources.preprocessors.jax import RunningStandardScaler

CONFIG = {
    "experiment": {
        "name":             "syncmixer_continuous_coord_v1",
        "agent_type":       "syncmixer",
        "directory":        "runs",
        "wandb":            True,
        "wandb_kwargs": {
            "project": "marl-comm",
            "tags":    ["syncmixer", "continuous_coord"],
        },
