# fmt: off
"""Scenario 4 — Heterogeneous hand-off pressure.

Changes vs nocomm baseline:
  * agent_speed_options    = (1, 3, 1, 3)  — alternating slow/fast
  * agent_capacity_options = (3, 1, 3, 1)  — alternating high/low cap
    so each fast agent (cap=1) can carry only one item but moves 3×, while
    slow agents (cap=3) act as accumulators.
  * num_goal_locations = 1, num_shelves = 8 — single delivery cell forces
    queueing; many shelves keep work supply high.
  * Step penalty halved so longer hand-off chains don't dominate the
    return.

Hypothesis (analysis.md §5, item 4):
  Knowing peer's `carrying` and `phase` is critical for routing items.
  no_comm hides phase from MAPPO; comm methods can hand off explicitly.
  Expected: MAPPO struggles to coordinate fast<->slow pairs; comm methods
  open a 15-25 % reward gap (Overcooked-style hand-off, Carroll 2019).
"""
...
CONFIG = {
    "experiment": {
        "name":             "mappo_warehouse_handoff_v1",
        "agent_type":       "mappo",
        "directory":        "runs",
        "wandb":            True,
        "wandb_kwargs": {
            "project": "marl-comm",
            "tags":    ["mappo", "warehouse", "handoff", "scenario4"],
        },
