...
CONFIG = {
    "experiment": {
        "name":             "mam_warehouse_team_sync_v2",
        "agent_type":       "mam",
        "directory":        "runs",
        "wandb":            True,
        "wandb_kwargs": {
    ...
    "mam": {
        "rollouts":        256,
        "learning_epochs": 10,     # 10 caused ratio explosion (grad_norm 5× MAPPO's)
        ...
        "grad_norm_clip":         0.5,   # tightened from 0.5: MAM grad_norm was 5× MAPPO
        ...
        "rewards_shaper":       lambda rewards, *args: jnp.clip(rewards, -5.0, 40.0),  # clip extremes; None caused large negative advantages destabilizing training
        ...
        "d_state":     32,     # scaled from 32: 2× for 4× more agents; maintains SSM memory across 16-step AR chain
        "d_conv":      4,      # paper default; d_conv=1 is degenerate (pointwise, no causal context)
        "delta_rank":  32,     # scaled from 16: match d_state order of magnitude for richer input-dependent transitions
    },
