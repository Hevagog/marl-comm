# fmt: off
"""Scenario 1 — Team-synchronized delivery bonus.

Drop-in clone of mappo_warehouse_nocomm.py with two env-side changes:
  * `enable_team_delivery_bonus = True`
  * `team_delivery_window = 20`, `team_delivery_bonus = 5.0`

Hypothesis (analysis.md §5, item 1):
  When delivery reward depends on a peer also delivering inside a 20-step
  window, MAPPO's per-agent advantage estimator under-credits deliveries
  that lack temporal cooperation.  CommFormer / MAM / MAPPO-Hopfield
  should open a measurable reward gap because their attention/Hopfield
  modules have peer-phase context to time hand-offs.

  Expected: MAPPO reward plateau ≈ baseline (no extra bonus captured),
  comm methods plateau ≈ baseline + 30–40 (capture team bonus).

Run: `uv run python -m src.cli --config configs/warehouse/mappo_warehouse_team_sync.py`
"""
...
CONFIG = {
    "experiment": {
        "name":             "mappo_warehouse_team_sync_v1",
        "agent_type":       "mappo",
        "directory":        "runs",
        "wandb":            True,
        "wandb_kwargs": {
            "project": "marl-comm",
            "tags":    ["mappo", "warehouse", "team_sync", "scenario1"],
        },
    ...
    "mappo": {
        ...
        # Team-sync bonus can push instantaneous reward above the original
        # +20 cap; widen the shaper ceiling to avoid clipping the signal.
